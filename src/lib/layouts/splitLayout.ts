...
export function splitLayoutSpec(slide: Slide, theme: Theme, totalSlides: number, forceTier?: FontTier): LayoutSpec {
  ...
  elements.push({
    ...
    style: { fontSize: headlineFont, fontWeight: 'bold', color: theme.typography.body, lineHeight: 1.3 },
  });
  leftY += headlineH + 8;
  ...
  elements.push({ id: `counter_${n}`, type: 'text', x: MARGIN_L, y: 405 - MARGIN_B - 14, width: 50, height: 14, content: `${n} / ${totalSlides}`, style: { fontSize: 11, color: theme.typography.muted, opacity: 0.6 } });
